...
AI 管家是 RippleFlow 的"运营大脑"，负责主动维护知识库健康和提升信息使用效率。

### 核心职责

| 职责 | 触发条件 | 输出 | 频率 |
|------|----------|------|------|
| 每周知识快报 | 定时（周一 9:00） | 群聊推送 | 每周 |
| 待办到期提醒 | due_date - 1 天 | 群聊@提醒 | 每日检查 |
| 敏感授权状态更新 | 授权状态变化 | 当事人通知 | 实时 |
| 问答反馈收集 | 用户使用问答后 | 反馈请求 | 实时 |
| 知识库健康报告 | 定时（每月 1 日） | Dashboard 展示 | 每月 |
| 孤儿线索检测 | 无关联消息的线索 | 管理员通知 | 每周 |
| 摘要质量预警 | AI 摘要置信度低 | 当事人确认请求 | 实时 |
