...
| 12 | [设计答记者问](./12_design_qa.md) | 设计意图澄清、关键决策记录、设计信条 | 设计参考 |
| 13 | [分阶段实施路线](./13_implementation_phases.md) | Phase 0-3 完整实施路线、表归属、风险缓解 | 项目管理 |
| PRD | [FAQ 知识沉淀系统 PRD](./群聊知识沉淀与FAQ智能演进系统_PRD.md) | FAQ 自动生成、演进机制、里程碑与 KPI | 产品 / 开发 |
| plans | [nullclaw 集成配置](./plans/nullclaw-integration-config.md) | nullclaw 集成设计、事件协议、权限配置 | SDD / 运维 |
...
| `qa_faq` | 问题解答 / FAQ | 技术问答、排查方法；由 nullclaw 自动沉淀为结构化 FAQ 知识库 | 90 天 |
...
| **FAQ 知识库** | 章节树浏览、全文搜索、答案溯源、用户反馈；由 nullclaw 自动生成并演进 |
| 搜索问答 | 自然语言提问，FAQ 优先命中，LLM 兜底综合回答 |
| 敏感授权 | 处理待授权的敏感内容请求，实时查看授权进度 |
| 当事人修改 | 修正摘要内容，可选同步回群 |
| 个人贡献 | 查看个人知识贡献统计 |
| 管理后台 | 白名单、类别管理、**FAQ 审核队列**、统计面板（管理员） |
...
├── skills/        # 技能模板（管家可优化）
├── insights/      # 自省沉淀（管家维护）
└── extensions/    # 插件脚本（新能力扩展，由 claw 团队编写）
```
...
| 角色 | 职责 |
|------|------|
| AI 管家 | 发现平台能力缺口，输出标准格式 PRD |
| claw 开发团队 | 评审 PRD，编写并注册插件（event_hook / nullclaw_script） |
| 平台管理员 | 审核并激活插件 |

```
管家发现缺口 → 输出 PRD → claw团队编写插件 → 管理员激活 → 管家调用
```

**两种调整路径：**
- **提示词调整**：修改 duties/ / skills/ / insights/ 中的 Prompt，无需部署，影响管家决策行为
- **插件扩展**：编写新的 extension 脚本，需要注册和审核，扩展平台能力边界
...
└─────────────────────────────────────────────────────────────────────┘
           ...
           ▼
┌─────────────────────────────────────────────────────────────────────┐
│  nullclaw Watchdog（独立进程，Phase 1 必须交付）                    │
│  监控 nullclaw 健康 → 检测故障 → 自动重启 → 通知平台就绪           │
│  与 nullclaw_pending_events 配合：重启期间事件不丢失                │
└─────────────────────────────────────────────────────────────────────┘
```
