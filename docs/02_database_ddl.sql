...
CREATE TYPE notification_type AS ENUM (
    ...
    'reminder',               -- 提醒
    -- 订阅触发通知（subscription_events 驱动）
    'document_published',     -- 你关注的人/关键词有新文档发布
    'shared_link_shared',     -- 你关注的人/关键词分享了外部链接
    'todo_updated',           -- 你关注的任务有更新
    'plan_milestone_reached', -- 你关注的计划卡达到里程碑
    'new_thread',             -- 你关注的类别/关键词出现新话题线索
    'user_new_contribution'   -- 你关注的用户有新贡献（文档/链接/线索）
);
...
CREATE TABLE queued_notifications (
    id                   UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id              VARCHAR(255) NOT NULL,
    notification_type    notification_type NOT NULL,  -- 使用统一枚举，替代原 event_type
    payload              JSONB NOT NULL DEFAULT '{}',
    priority             INTEGER NOT NULL DEFAULT 5,
    -- 订阅溯源（可选）：便于"取消订阅后撤回历史通知"等操作
...
CREATE TABLE user_documents (
    ...
    visibility                VARCHAR(20) NOT NULL DEFAULT 'team',
        -- 'private'   仅作者可见
        -- 'followers' 在 user_subscriptions 中关注了该作者（subscription_type='user'）的用户可见
        --             访问控制查询：EXISTS (SELECT 1 FROM user_subscriptions
        --                           WHERE user_id=<viewer_id> AND subscription_type='user'
        --                           AND target_id=<author_id> AND is_active=TRUE)
        -- 'team'      同群组成员可见（需 group_id 非空，否则同 public）
        -- 'public'    所有登录用户可见
...
CREATE TABLE shared_links (
    ...
    tags                      TEXT[] DEFAULT '{}',
    visibility                VARCHAR(20) NOT NULL DEFAULT 'team',
        -- 同 user_documents.visibility，followers 同样基于 user_subscriptions(type='user') 判断
    -- 管家建议字段
    butler_suggested_category VARCHAR(100),
    butler_suggested_tags     TEXT[] DEFAULT '{}',
...
CREATE TABLE butler_suggestions (
    ...
    entity_id     UUID,                        -- NULL = 实体创建中（尚未保存）
        -- 实体保存后，由业务层调用 POST /api/v1/butler/suggest/feedback
        -- （或在创建 API 中内联）将 entity_id 回填：
        -- UPDATE butler_suggestions SET entity_id=$1
        -- WHERE user_id=$2 AND entity_type=$3 AND entity_id IS NULL
        -- AND created_at > NOW() - INTERVAL '1 hour';
    field         VARCHAR(100) NOT NULL,
        -- 'category' | 'tags' | 'title' | 'description' | 'summary' | 其他字段名
        -- tags 字段：value 为单个 tag 字符串（非逗号分隔列表）
        --           suggestions 数组中每条 value = 一个 tag，可多选采纳
    ...
    -- auto_apply 场景：confidence > 0.9 时系统静默填入
    -- 用户保存实体时，业务层检查是否有 auto_apply 记录且 applied=FALSE，
    -- 自动发送 feedback（applied=TRUE, applied_value=最终字段值），实现隐式反馈闭环
    -- 若用户事后修改了该字段，视为用户覆盖，下次建议时降低同类推荐的置信度参考权重
    ai_applied    BOOLEAN NOT NULL DEFAULT FALSE,  -- TRUE=系统自动填入（auto_apply），区分人工确认
    created_at    TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE INDEX idx_butler_sugg_entity ON butler_suggestions (entity_type, entity_id)
    WHERE entity_id IS NOT NULL;
CREATE INDEX idx_butler_sugg_pending ON butler_suggestions (user_id, entity_type, created_at DESC)
    WHERE entity_id IS NULL;              -- 快速查找"创建中"的孤儿建议
