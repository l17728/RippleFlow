...
**初审日期**: 2026-03-02（v0.6 基准）
**补充审查**: 2026-03-05（v0.8 补丁，见 §十三）
**审查范围**: 全部设计文档
**审查视角**: 产品经理 / 软件架构师 / 系统设计专家 / 信息学专家

> **阅读说明**：§一至§十二 为 v0.6 初审内容，原文保留（历史参考价值）。
> §十三 为 v0.7/v0.8 补充审查，覆盖新增功能评估、原有建议落地跟踪、新发现风险。
> **当前有效评分见 §十三·13.1**（已基于 v0.8 重新校准）。
...
## 十三、v0.7/v0.8 补充审查（2026-03-05）

> 本节基于 v0.8 全量文档（75 表 / 138 API / 26 Protocol）进行补充评审，
> 重点跟踪原建议落地情况，评估新功能设计质量，识别新引入风险。
>
> **阅读说明**：§一至§十二 为 v0.6 初审内容，原文保留（历史参考价值）。
> 当前有效结论见本节。
...
| 维度 | v0.6 评分 | v0.8 评分 | 变化原因 |
|------|-----------|-----------|----------|
| **需求完整性** | ⭐⭐⭐⭐☆ | ⭐⭐⭐⭐⭐ | v0.7 订阅/内容发布填补产品空白；v0.8 全链路推演修复 17 项边缘场景 |
| **架构合理性** | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | Pipeline 防死循环、有序重试、通知时序修复，核心架构更严密 |
| **技术可行性** | ⭐⭐⭐⭐☆ | ⭐⭐⭐⭐☆ | LLM 批量分类（§22）部分落地；全面成本优化仍需时间 |
| **用户体验** | ⭐⭐⭐☆☆ | ⭐⭐⭐⭐☆ | AI 智能辅助输入（§44）显著提升填表体验；订阅关注解决信息过载 |
| **可扩展性** | ⭐⭐⭐⭐☆ | ⭐⭐⭐⭐⭐ | butler_push_config 动态配置无需重启；26 个 Protocol 接口完整；10 种订阅类型可扩展 |
| **运维友好度** | ⭐⭐⭐☆☆ | ⭐⭐⭐⭐☆ | ApiKey 认证体系（GAP-9）、pending_events 有序重试（GAP-10）、推送配置热更新 |

**综合提升**：从 v0.6 的"架构扎实但体验粗糙"，进化为 v0.8 的"全链路严密且体验精细化"。
...
| 原建议 | 落地情况 |
|--------|----------|
| §3.3 防止消息处理死循环 | `pipeline_start_stage` 字段 + `run(start_stage=1)` 防 Stage0 重入（GAP-1）|
| §3.3 Webhook 幂等性 | `ON CONFLICT (external_msg_id) DO NOTHING`（GAP-8）|
| §2.3 机器人消息过滤 | `is_bot_message` + `ingest()` 提前过滤（GAP-7）|
| §3.3 reference_data 去重 | `deprecate_old_reference_data()` 触发器自动废弃同 key_name 旧记录（GAP-6）|
| §2.4 通知时序与质量 | 订阅通知在 Stage5 摘要就绪后触发（GAP-16）；keyword 匹配异步化 |
| §10.1 参考数据搜索 | `ISearchService.find_reference()` 新增，含敏感值访问控制（GAP-12/13）|
| §六 内部服务认证 | `apiKeyAuth` 安全方案 + `verify_api_key()`（GAP-9）|

#### ⏳ 部分落地

| 原建议 | 当前状态 |
|--------|----------|
| §3.1 LLM 成本优化（批量分类） | §22 批量 Prompt 已设计，待实现 Batch API 调用 |
| §3.2 全文检索性能 | 仍用 PostgreSQL FTS，`simple` 字典，中文精度不足（D-04）|
| §3.4 数据归档策略 | Phase 3 规划中，`archive_status` 字段已保留 |
| §七 深度可观测性 | `user_behavior_logs` / `butler_tool_calls` 表存在，无 APM 接入方案 |

#### ❌ 维持原状（已明确不采纳）

| 原建议 | 理由 |
|--------|------|
| §十 Elasticsearch 接入 | 团队决策维持 PostgreSQL FTS，控制复杂度 |
| §六 WebSocket 实时更新 | Phase 3 后期，当前 polling 够用 |
| §六 混合语义检索 | 无向量扩展为明确设计约束，维持 FTS + LLM 兜底 |
...
**亮点**：
- 类型完整（user/thread/category/keyword/todo/resource/event/document/shared_link/workflow）
- `filter_criteria JSONB` 支持精细过滤，避免通知轰炸
- `get_followable_targets()` 提供前端搜索入口

**风险**：keyword 订阅使用 `simple` 字典，中文匹配精度不足；订阅类型过多，新用户教育成本高。

**建议**：新用户首次登录时，管家根据 LDAP 属性（部门/技术栈）推送"智能关注建议"，一键确认降低配置门槛。

#### 13.3.2 内容发布（文档 + 链接卡片）⭐⭐⭐⭐☆

**亮点**：
- 两种形态定位清晰；`source_thread_id` 将群聊讨论与正式文档关联，知识链路完整
- OG 元数据自动抓取 + 降级策略合理

**风险**：
- `user_documents.content`（TEXT 无限制）对大型文档（> 100KB）存在查询性能隐患
- 内网系统（飞书/企业 Notion）OG 抓取失败率高，需统计并转为手动提示

**建议**：业务层校验 content ≤ 200KB；维护 `og_fetch_stats` 表统计域名失败率，高失败率域名提示用户手动填写。

#### 13.3.3 AI 智能辅助输入（§44）⭐⭐⭐⭐⭐

**亮点**：
- 跨切面设计模式，一套 API 覆盖全系统所有实体，架构简洁
- 三级建议强度（auto_apply/highlight/list）分层精准
- `context_hash` 防重复触发；孤儿建议回填机制（`link_entity`）解决了创建流程时序问题

**风险**：
- `auto_apply`（confidence > 0.9）静默填入，分类错误用户可能不察觉——需监控"填入后被修改率"
- `butler_suggestions` 表随使用增长，建议设置 90 天 TTL 定期清理（未采纳记录）

---

### 13.4 v0.8 全链路推演方法论评审

**这是本版本最重要的质量提升手段。**

通过 13 个场景时序推演（用户视角 + AI 管家视角），识别出 17 项原本难以被常规 Review 发现的漏洞：

| 发现类型 | Code Review | 时序推演 | 典型案例 |
|----------|-------------|----------|----------|
| 接口参数缺失 | ✅ | ✅ | — |
| 跨服务时序 Bug | ❌ | ✅ | GAP-16：通知在摘要就绪前触发 |
| 并发一致性问题 | ❌ | ✅ | GAP-10：宕机恢复后事件乱序重放 |
| 重入死循环风险 | ❌ | ✅ | GAP-1：敏感消息重授权触发 Stage0 死循环 |
| 默认值业务隐患 | ❌ | ✅ | GAP-4：action_item todo 默认 private，团队不可见 |
| 认证遗漏（攻击面） | ⚠️ | ✅ | GAP-9：/internal 端点无鉴权 |

**建议**：将时序推演固化为每个 Phase 验收前的必做评审环节（类比 Architecture Decision Record）。

**17 项 GAP 分布规律**：
- CRITICAL（3）：多服务交互边界的流程防护缺失
- HIGH（8）：认证/幂等/有序性等机制完整性缺口
- MEDIUM（6）：默认值/文本规范/配置动态化等体验精化

---

### 13.5 新引入风险

#### R-01：butler_suggestions 数据膨胀

200 用户 × 日均 10 次触发 ≈ 2000 条/天，1 年约 70 万条。
**建议**：每日清理超过 90 天的未采纳建议（`applied=FALSE`），保留 365 天的已采纳记录用于学习分析。

#### R-02：多线索消息 Stage5 幂等性

多线索消息 notify_nullclaw 使用 `thread_updates[n]` 格式，nullclaw 中途崩溃时部分线索无 Stage5。
pending_events 重试会重发完整 payload，但 nullclaw 需能识别"已处理线索"并跳过。
**建议**：nullclaw 侧在处理 `thread_updates` 时，维护 `processed_thread_ids` 集合实现幂等。

#### R-03：butler_push_config 与 nullclaw 内置 cron 调度叠加

管理员修改 `schedule` 字段，若 nullclaw 内置 cron 仍并行运行，同一天可能发两次日报。
**建议**：nullclaw 启动时读取 `butler_push_config`，`schedule != null` 时替换内置 cron（非叠加）。
需在 `08_ai_butler_architecture.md` 补充"配置 schedule 优先级 > 内置 cron"说明。

#### R-04：keyword 订阅 FTS 在高负载下的延迟

500+ 条 keyword 订阅 × 每条新消息 = 500+ 次 FTS 查询串行执行，延迟可能达数分钟。
**建议**：维护 `keyword → [user_id]` 内存倒排索引，消息入库时一次 `tsvector` 转换后与内存集合 intersect，时间复杂度从 `O(n_keywords × n_messages)` 降至 `O(n_messages)`。

---

### 13.6 当前有效实施优先级（v0.8 修订）

在 v0.6 §十 优先级建议基础上，修订如下：

**新增高优先级**（Phase 1 验收前必做）：

| 项目 | 原因 |
|------|------|
| keyword 订阅 FTS 性能压测（R-04） | 200 keyword × 1000 消息/天场景验证延迟 |
| nullclaw Stage5 幂等机制（R-02） | 多线索消息中断恢复场景必须通过 E2E 测试 |
| butler_suggestions TTL 清理任务（R-01） | 防止上线后数据膨胀 |
| butler_push_config 调度优先级文档（R-03） | 明确 nullclaw 内置 cron 覆盖规则 |

**调整优先级**（从高降为中）：

| 项目 | 原因 |
|------|------|
| "增强消息处理可靠性"（§3.3） | 已通过 GAP-1/8/10 修复完成，核心问题已解决 |

---

### 13.7 补充审查总结

**v0.7/v0.8 是 RippleFlow 从"概念验证"到"工程就绪"的关键飞跃**：

1. **产品层面**：订阅、内容发布、AI 辅助输入三大功能落地，用户价值显著扩展
2. **工程层面**：17 项 Pipeline 防护机制修复，系统鲁棒性大幅增强
3. **架构层面**：ApiKey 认证体系、动态推送配置、有序事件重放，运维操作面成熟
4. **方法论层面**：时序推演作为设计验证手段被验证有效，建议固化为标准评审环节

**Phase 1 验收前最高风险（必须解决）**：
- keyword FTS 高负载延迟（R-04）
- multi-thread 消息 Stage5 幂等性（R-02）

**下一轮审查触发条件**：Phase 1 验收时（首批真实用户接入后），基于实际运行数据更新评分。

---

**补充审查完成时间**: 2026-03-05
**补充审查人**: Claude AI（基于 v0.8 全量文档分析）
**原评审人**: Kimi (AI Product Architect)（2026-03-02 初审）
